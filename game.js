...
// =====================
// ZONES (APP STORE FEEL)
// =====================
const zones = [
  { name: "Forest Zone", color: "#4CAF50" },
  { name: "Safari Zone", color: "#FF9800" },
  { name: "Water Zone", color: "#03A9F4" }
];
...
// =====================
// AUDIO (FIXED - NO ARABIC CONFUSION)
// =====================
function speak(text) {
...
function check() {
  ...
  if (choice === state.target) {
    ...
    // 🌟 PERMANENT ZOO ANIMAL (KEY UPGRADE)
    releasedAnimals.push({
      type: animal,
      x: cage.x,
      y: cage.y,
      tx: cage.x + 250 + Math.random() * 300,
      ty: cage.y - 120 + Math.random() * 200
    });
...
function drawPaths() {
  ...
  cages.forEach((c, i) => {
    const x = c.x * scale - state.cameraX;
    const y = c.y * scale + 120 * scale;
...
// =====================
// GATES (PROGRESSION FEEL)
// =====================
function drawGates() {
  for (let i = 1; i < cages.length; i++) {

    const prev = cages[i - 1];

    const x = (prev.x + 450) * scale - state.cameraX;
    const y = 520 * scale;

    const unlocked = i <= state.currentIndex;

    ctx.fillStyle = unlocked ? "#FFD54F" : "#666";
...
function drawCages() {
  ...
  for (const cage of cages) {
    ...
    if (cage.shake > 0) {
      cage._shakeX = Math.sin(Date.now() * 0.08) * cage.shake;
      cage.shake *= 0.85;
    } else {
      cage._shakeX = 0;
    }
    ...
    if (img.complete) {
      ctx.drawImage(img, x - size / 2 + cage._shakeX, y - size / 2, size, size);
    }
  }
}
...
function drawCompletion() {
  ...
  ctx.fillText("All Animals Now Live in the Zoo", canvas.width / 2, canvas.height / 2 + 40);
}
...
window.addEventListener("keydown", (e) => {
  ...
  if (e.key === "ArrowRight") state.selected = Math.min(2, state.selected + 1);
